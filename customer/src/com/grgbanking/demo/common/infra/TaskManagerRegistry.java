package com.grgbanking.demo.common.infra;
...
public class TaskManagerRegistry {
    ...
    public static void waitAll(final Context context, final Runnable done, final int max, final int interval) {
		...
		com.grgbanking.demo.common.infra.Handlers.sharedHandler(context).postDelayed(new Runnable() {
			int count;

			@Override
			public void run() {
				if (!idle() && count++ < max) {
					com.grgbanking.demo.common.infra.Handlers.sharedHandler(context).postDelayed(this, interval);
				} else {
					done.run();
				}
			}
